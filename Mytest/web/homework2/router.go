...
func (r *router) addRoute(method string, path string, handleFunc HandleFunc, mdls ...Middleware) {
	...
	// 跟節點重複註冊
	if path == "/" {
		if root.handler != nil && handleFunc != nil {
			panic("web: 路由不能重複註冊 [/]")
		}
		if handleFunc != nil {
...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	for _, seg := range segs {
		...
		for _, candidate := range candidates {
			// 收集所有能匹配當前段的子節點
			matchedChildren := r.getAllMatchedChildren(candidate, seg)
			...
			for _, child := range matchedChildren {
				...
				nextCandidates = append(nextCandidates, child)
			}
		}
...
// 獲取所有能匹配當前段的子節點
func (r *router) getAllMatchedChildren(n *node, seg string) []*node {
	var matched []*node
	...
	if n.regexChild != nil && n.regexChild.regex != nil && n.regexChild.regex.MatchString(seg) {
		matched = append(matched, n.regexChild)
	}

	// 3. 普通參數匹配
	if n.paramChild != nil {
		matched = append(matched, n.paramChild)
	}

	// 4. 通配符匹配
	if n.starChild != nil {
		matched = append(matched, n.starChild)
	}
...
func (r *router) findRoute(method string, path string) (*matchInfo, bool) {
	...
	return &matchInfo{
		n:          root,
		pathParams: pathParams,
		mdls:       r.findMdls(r.trees[method], segs),
	}, true
}
